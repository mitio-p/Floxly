...
io.on("connection", (socket) => {
  socket.on("join_conversation", async (data) => {
    if (socket.handshake.headers.cookie) {
      ...
      if (
        (await ConversationsSchema.findById(data)).participants.includes(userId)
      ) {
        console.log("joined");
        socket.join(data);
      }
  ...
  socket.on("send-message", async (data) => {
    if (socket.handshake.headers.cookie) {
      ...
      if (
        (await ConversationsSchema.findById(data.room)).participants.includes(
          userId
        )
      ) {
        socket
          .to(data.room)
          .emit("recieve-message", { ...data, isDelivered: true });
      }
    }
  });
...
app.post("/createConversation", gatherUserInfo, async (req, res) => {
  ...
  if (
    !(await ConversationsSchema.findOne({
