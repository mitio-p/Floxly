        ...
        <div className={classes.fieldsContainer}>
          <div>
            <input
              ...
              ref={emailInput}
            />
            {actionData ? actionData?.errors?.email && <p>{actionData.errors.email}</p> : null}
          </div>
          {mode !== 'forgotPassword' && (
            <div>
              <input
                ...
                ref={passwordInput}
              />
              {actionData ? actionData?.errors?.password && <p>{actionData.errors.password}</p> : null}
            </div>
          )}
          {mode !== 'login' && mode !== 'forgotPassword' && (
            <>
              <div>
                ...
                {actionData ? actionData?.errors?.fullname && <p>{actionData.errors.fullname}</p> : null}
              </div>
              <div>
                <input type="text" placeholder="Username" name="username" disabled={isSubmitting} required />
                {actionData ? actionData?.errors?.username && <p>{actionData.errors.username}</p> : null}
              </div>
            </>
          )}
        </div>
...
export async function loader() {
  const response = await authFetch('http://localhost:3000/auth/user', { credentials: 'include' });
...
export async function action({ request }) {
  ...
  if (searchParams.get('mode') === 'login') {
    const response = await fetch('http://localhost:3000/auth/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', authorization: `Bearer ${tokenSevice.getToken()}` },
      credentials: 'include',
