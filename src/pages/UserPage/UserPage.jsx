import { json, Link, useLoaderData, useSearchParams } from 'react-router-dom';
...
export default function UserPage() {
  ...
  async function handleFollow() {
    const response = await fetch(`http://localhost:4000/user/follow/${loaderData.user.username}`, {
      credentials: 'include',
    });
    if (response.ok) {
      setFollowing(true);
  ...
  async function handleUnfollow() {
    const response = await fetch(`http://localhost:4000/user/unfollow/${loaderData.user.username}`, {
      credentials: 'include',
    });
    if (response.ok) {
      setFollowing(false);
  ...
  return (
    <main className={classes.userContainer}>
      ...
      <div className={classes.userInfo}>
        ...
        <div className={classes.userStats}>
          <div>
            ...
            {isCurrentUser ? (
              <>
                <button>Edit Profile</button> <button>Settings</button>
              </>
            ) : (
      ...
      {!loaderData.user.gallery.length > 0}
      {isCurrentUser ? (
...
export async function loader({ params }) {
  ...
  const response = await fetch(`http://localhost:4000/user/${username}`, { credentials: 'include' });
